  ...
  return (
    <div
      className={`relative flex h-full w-full flex-col overflow-hidden
                  ${dark ? 'bg-[#0A0A0F]' : 'bg-[#faf9fc]'} ${fgCls(dark)} ${className}`}
    >
      ...
      <div
        ...
        className={`pointer-events-none absolute inset-0
                    ${dark
                      ? 'bg-[radial-gradient(1100px_520px_at_78%_-10%,rgb(91_45_140/0.28),transparent_65%),radial-gradient(900px_480px_at_12%_110%,rgb(74_168_201/0.16),transparent_60%)]'
                      : 'bg-[radial-gradient(1100px_520px_at_78%_-10%,rgb(91_45_140/0.10),transparent_65%),radial-gradient(900px_480px_at_12%_110%,rgb(111_195_221/0.18),transparent_60%)]'}`}
      ></div>

      {/* Kopfzeile */}
      <header className="relative z-30 flex items-center justify-between gap-4 px-6 pt-4 max-lg:px-4 max-lg:pt-3">
        <div className="flex items-baseline gap-3">
          <span className="text-[21px] font-bold tracking-tight first-letter:text-[#8b5cf6]">K-Aqua</span>
          <span className={`h-4 w-px self-center max-lg:hidden ${dark ? 'bg-white/15' : 'bg-black/10'}`}></span>
          <span className={`text-[13.5px] font-medium uppercase tracking-[0.14em] max-lg:hidden ${mutedCls(dark)}`}>
            Sprachen-Hub
          </span>
        </div>
        <div className="flex gap-2">
          ...
          >
            ...
            <span className={`text-[11px] font-semibold uppercase tracking-widest max-lg:hidden ${faintCls(dark)}`}>
              Aktiv
            </span>
      ...
      {/* Titel */}
      <div className="pointer-events-none relative z-10 px-6 pt-6 text-center max-lg:pt-3">
        <h1 className="text-[clamp(26px,3.4vw,44px)] font-bold leading-[1.1] tracking-tight">
          Wähle deine Sprache
        </h1>
        <p className={`mt-1.5 text-sm max-lg:text-[12.5px] ${mutedCls(dark)}`}>
      ...
      <div ref={hostRef} className="relative z-10 min-h-0 flex-1">
        ...
        <div className={`absolute bottom-4 left-6 text-xs tracking-wide max-lg:hidden ${faintCls(dark)}`}>
