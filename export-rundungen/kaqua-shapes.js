/* K-Aqua Shapes — SVG-Clip-Definitionen + Liquid-Engine v2
   Einmal pro Seite einbinden: <script src="kaqua-shapes.js" defer></script>

   Teil 1: injiziert clipPath-Defs (objectBoundingBox) für die Clip-Formen.
   Teil 2: Offscreen-Pause für Endlos-Animationen (data-kq-pause).
   Teil 3: Liquid-Engine — animiert die GESAMTE Oberfläche jedes Elements
   kontinuierlich und individuell (Mischung aus Wasser & Lavalampe):
     · Silhouetten-Wobble: border-radius-Ecken bzw. Clip-Pfadpunkte laufen
       auf überlagerten Sinus-Feldern mit eigenem Seed pro Element
     · Slosh: das Bild schwappt träge und phasenversetzt im Container
     · Drift: Mikro-Rotation, -Translation, -Atmung wie in einer Lavalampe
     · Interaktiv: Cursor-Geschwindigkeit pumpt Energie hinein (Wellen
       werden höher, klingen exponentiell ab), die Kante wölbt sich lokal
       zum Cursor; Scroll-Geschwindigkeit lässt alles schwappen
   Markierung: class="kq-liquid" (Formen und Bänder). Bei
   prefers-reduced-motion bleibt alles vollständig statisch. */
(function () {
  'use strict';
  ...
  function inject() {
    if (document.getElementById(ID)) { defsEl = document.getElementById(ID).firstChild; return; }
  ...
  /* Endlos-Animationen außerhalb des Viewports pausieren */
  ...
  function parsePath(d) {
    // Zerlegt M/L/C/Z-Pfade in Segmente mit Zahlenlisten (Unit-Space 0..1)
    var tokens = d.match(/[MLCZ]|-?\.?\d+(?:\.\d+)?(?:e-?\d+)?/gi) || [];
    var segs = [], i = 0;
    var need = { M: 2, L: 2, C: 6 };
    while (i < tokens.length) {
      var cmd = tokens[i++];
      if (cmd === 'Z' || cmd === 'z') { segs.push({ c: 'Z', n: [] }); continue; }
  ...
  function motionInit() {
    ...
    /* Auftauchen: einmalig beim Eintritt */
    ...
    var tracked = [];
    var liqN = 0;
    document.querySelectorAll('.kq-ix, .kq-lava, .kq-float, .kq-liquid').forEach(function (el) {
      var t = {
        el: el,
        ...
        par: 0, tpar: 0, e: 0,
        vis: false, hov: false,
        mode: null, segs: null, pathEl: null, corners: null, edgeBase: 72, tiltBase: -1.2,
        sp: 0.45 + ((tracked.length * 37) % 11) / 22  /* 0.45..0.95 rad/s */
      };
    ...
    function initLiquid(t) {
      var el = t.el, cls = el.classList;
      /* Clip-Form? -> eigenen clipPath klonen und Punkte animieren */
      ...
      if (t.isBand) {
        /* Bänder: Kantenhöhe bzw. Neigung oszillieren (sofern keine
           CSS-Animation die Variable ohnehin führt) */
      ...
      /* Radius-Form: 8 Eckwerte kontinuierlich morphen */
      t.mode = 'radius';
    ...
    var vio = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) { if (e.target._kqT) e.target._kqT.vis = e.isIntersecting; });
    }, { rootMargin: '160px' });
    tracked.forEach(function (t) { vio.observe(t.el); });

    /* Cursor: Position + Geschwindigkeit (Energie-Eintrag) */
    var pX = -1e4, pY = -1e4, lpX = -1e4, lpY = -1e4, pActive = false;
    document.addEventListener('pointermove', function (ev) {
      pX = ev.clientX; pY = ev.clientY; pActive = true;
    }, { passive: true });
    document.addEventListener('pointerleave', function () { pActive = false; }, { passive: true });

    /* Scroll-Energie (Schwappen) */
    ...
    function frame(now) {
      ...
      for (var i = 0; i < tracked.length; i++) {
        ...
        /* Cursor-Verfolgung, Hover, Energie */
        var inside = pActive &&
        ...
        /* ---- kontinuierliche Flüssig-Simulation ---- */
        if (!t.lq) continue;
        ...
        if (t.mode === 'radius') {
          ...
          for (var k = 0; k < 4; k++) {
            ...
            /* lokale Wölbung zur Cursor-Ecke */
            var bulge = 0;
            if (t.hov) {
        ...
        /* Lavalampen-Drift + Slosh (alle Liquid-Elemente außer Bänder) */
        if (!t.isBand) {
          var st = el.style;
          ...
          /* Bild schwappt phasenversetzt (Trägheit) */
