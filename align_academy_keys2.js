...
function alignArray(myArr, deArr, keyMapping) {
  ...
  return myArr.map((item, i) => {
    ...
    for (const [myKey, deKey] of Object.entries(keyMapping)) {
       ...
       if (item[myKey] !== undefined) newItem[deKey] = item[myKey];
       else if (item[deKey] !== undefined) newItem[deKey] = item[deKey];
    }
    ...
    for (const k in deItem) {
       if (k === 'year' || k === 'label') {
          // just use German original since it's probably standard "PHASE 01", "2023", etc.
          finalItem[k] = deItem[k];
       } else if (k === 'p2' && newItem[k] === undefined && item['d'] !== undefined) {
          finalItem[k] = ''; // p2 might be empty if it wasn't translated
       } else {
...
if (academy.schulungen) {
  ...
  academy.schulungen.timeline.items = alignArray(academy.schulungen.timeline.items, de.academy.schulungen.timeline.items, { title: 'title', desc: 'text' });
...
if (academy.zertifizierung) {
  ...
  academy.zertifizierung.timeline.items = alignArray(academy.zertifizierung.timeline.items, de.academy.zertifizierung.timeline.items, { title: 'title', desc: 'text' });
}
