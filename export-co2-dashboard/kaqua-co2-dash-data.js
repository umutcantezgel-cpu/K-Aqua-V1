/* K-Aqua CO2 Dashboard — Rechenmodell v3 (quellenbasiert):
   Massen aus realen Wanddicken (Kunststoff: SDR-Geometrie; Kupfer: EN-1057-Reihe;
   Edelstahl: Systemrohr-Reihe), Herstellungsfaktoren aus EPDs (A1–A3 komplett,
   keine separate Fertigungsenergie mehr), Betrieb über Darcy-Weisbach/Swamee-Jain
   mit alternder Rauheit, Recycling-Gutschrift als Anteil der Produktionslast
   (Modul D), Strommix UBA/Ember Stand 2025. Quellen: CO2_SOURCES. */
const CO2_DIAMETERS = [20, 25, 32, 40, 50, 63, 75, 90, 110, 125, 140, 160, 200, 250, 315, 400, 500, 630];
const CO2_SDR_CLASSES = [6, 7.4, 9, 11, 17];
...
/* hoursPerYear: Pumpen-Volllaststunden je Betriebsart (Annahme, dokumentiert):
   kalt = Druckerhöhung intermittierend; Kühlung = Umwälzung Kühlsaison;
   warm = Zirkulation nahezu durchgehend. eta = Pumpen-Gesamtwirkungsgrad. */
const CO2_OPERATING_MODES = [
...
const CO2_PUMP_ETA = 0.55;
const CO2_NU = 1.0e-6; /* kinematische Viskosität Wasser ~10–20 °C, m²/s */

/* productionFactor = GWP A1–A3 je kg (EPD); recyclingCreditShare = Modul-D-
   Gutschrift als Anteil der Produktionslast; roughnessMm = hydraulische Rauheit
   neu; roughnessGrowthMmPerYr = Inkrustation/Ablagerung (Annahme, konservativ);
   eolFactor = Rückbau/Entsorgung C1–C4 je kg (EPD-Größenordnung). */
const CO2_MATERIALS = [
...
/* Wanddicke in mm: Kunststoff nach SDR; Kupfer nach EN-1057-Reihe;
   Edelstahl nach Systemrohr-Reihe (pressfitting-typisch). */
function co2WallMm(material, d, sdr) {
...
function installBreakdown(material, ctx) {
  ...
  const transport = (mass / 1000) * ctx.transportKm * 0.09; /* kg CO2e je t·km Lkw */
...
function co2GridAt(ctx, calendarYear) {
...
/* Druckverlust nach Darcy-Weisbach, Reibungsbeiwert nach Swamee-Jain. */
function co2PressureLossPaPerM(material, d, sdr, flowLps, ageYears) {
  const wall = co2WallMm(material, d, sdr);
...
function operationYearKg(material, ctx, yearsSinceInstall, calendarYear) {
...
function eolBreakdown(material, mass) {
...
function computeFullResult(material, ctx, horizonYears) {
...
function co2Trees(kg) { return Math.max(0, Math.round(kg / 25)); }
function co2CarKm(kg) { return Math.max(0, Math.round(kg / 0.15)); }
function co2Fmt(kg) {
...
function findBreakEven(kaPoints, opPoints) {
...
function computePortfolioResult(material, ctxList, horizonYears) {
  const parts = ctxList.map((c) => computeFullResult(material, c, horizonYears));
  const points = parts[0].points.map((p, i) => ({ year: p.year, value: parts.reduce((a, r) => a + r.points[i].value, 0) }));
  const phaseValues = {};
  CO2_PHASES.forEach((ph) => { phaseValues[ph.id] = parts.reduce((a, r) => a + (r.phaseValues[ph.id] || 0), 0); });
  const evMap = {};
  parts.forEach((r) => r.events.forEach((e) => {
  ...
  return {
    ...
    events: Object.keys(evMap).map((k) => evMap[k]).sort((a, b) => a.year - b.year),
  };
}

/* Nutzungsdauer-Empfehlung aus realer Fließgeschwindigkeit (Erosionskorrosion-
   Richtwerte Kupfer): kalt ≤2,4 m/s, ≤60 °C/Zirkulation ≤1,5 m/s, dauerhaft
   >60 °C ≤0,9 m/s. Überschreitung → verkürzte Empfehlung (15–30 J. Praxisspanne). */
function co2LifespanGuide(materialId, opModeId, v) {
...
const CO2_DISCLAIMER = 'Berechnung auf Basis öffentlicher Quellen (EPDs nach EN 15804, UBA-Strommix 2025, Ember, DEHSt; Stand Juli 2026) und dokumentierter Annahmen — Quellen je Faktor im Methodik-Tab. Projektverbindlich sind produktspezifische Typ-III-EPDs.';
...
Object.assign(window, {
  CO2_DIAMETERS, CO2_SDR_CLASSES, CO2_REGIONS, CO2_OPERATING_MODES, CO2_MATERIALS, CO2_PHASES,
  co2WallMm, pipeMassKg, installBreakdown, operationYearKg, eolBreakdown, computeFullResult, co2GridAt,
  co2PressureLossPaPerM, findBreakEven, computePortfolioResult, co2LifespanGuide, CO2_PRESETS, CO2_EQUIVALENTS, CO2_SOURCES,
  co2Trees, co2CarKm, co2Fmt, CO2_DISCLAIMER, CO2_SEO_COPY,
});
