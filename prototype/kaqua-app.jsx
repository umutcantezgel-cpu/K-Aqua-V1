...
const NAV_PAGES = ['home', 'products', 'finder', 'co2', 'markets', 'trust'];
...
function Nav({ page, go, dark, setDark, lang, setLang }) {
  const { t } = useT();
  ...
  return (
    <React.Fragment>
          ...
          <nav className="k-nav-links" aria-label="Hauptnavigation">
            {NAV_PAGES.map((id) => (
              <button key={id} type="button"
                className={`k-nav-link ${page === id ? 'is-active' : ''}`}
                aria-current={page === id ? 'page' : undefined}
                onClick={() => go(id)}>
                {t.nav[id] || t.pages[id][0]}
              </button>
            ))}
          </nav>
      ...
      {menuOpen ? <MegaMenu page={page} go={go} onClose={() => setMenuOpen(false)} /> : null}
    </React.Fragment>
  );
}
...
function App() {
  ...
  return (
    <KLangContext.Provider value={{ lang, t: dict, dir: langDef.dir }}>
      ...
      <ScrollProgress />
      <Nav page={page} go={go} dark={t.dark} setDark={(v) => setTweak('dark', v)} lang={lang} setLang={setLang} />
