...
function RFQView({ go }) {
  ...
  const [d, setD] = useB({ type: null, meters: 1000, dims: [], timeline: null, region: null, name: '', company: '', email: '', phone: '', msg: '' });
  ...
  const valid = [d.type !== null, d.dims.length > 0, d.timeline && d.region, d.name && d.company && /.+@.+\..+/.test(d.email)];
      ...
      <section className="k-section" style={{ paddingTop: 0 }}>
        <div className="k-container" style={{ maxWidth: 860 }}>
          ...
          <BentoCard style={{ minHeight: 360 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 'var(--sp-3)', marginTop: 'auto', paddingTop: 'var(--sp-4)' }}>
              ...
              {step < 3
                ...
                : <KButton onClick={send} disabled={!valid[3]} icon={<Icons.ArrowUpRight size={18} />}>{L.send}</KButton>}
            </div>
          </BentoCard>
