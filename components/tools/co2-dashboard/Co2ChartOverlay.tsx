...
export function Co2TooltipPanel({ pct, title, rows, delta, note, pinned }: any) {
  ...
  return (
    <div className={`co2-panel is-${side}`} style={{ left: `${pct}%` }} role="status">
      ...
      {rows.map((r: any) => (
        <div className="co2-panel-row" key={r.id}><i style={{ background: r.color }}></i><span>{r.label}</span><span className="font-semibold">{r.value}</span></div>
      ))}
        ...
        <div className="co2-flag-pop" onPointerDown={(e) => e.stopPropagation()}>
          ...
          {ev.detail ? (
            <div className="co2-flag-pop-rows">
              <span>Neubau-Last</span><span className="font-semibold">{fmt(ev.detail.neubau)}</span>
              <span>Rückbau (brutto)</span><span className="font-semibold">{fmt(ev.detail.rueckbau)}</span>
              <span>Recycling-Gutschrift (angerechnet)</span><span className="font-semibold">−{fmt(ev.detail.gutschrift)}</span>
            </div>
          ) : null}
          <div className="co2-flag-pop-sum"><span>Ereignislast</span><span className="font-semibold">{fmt(ev.value)}</span></div>
        </div>
      ) : null}
    </div>
  );
