...
function Scene({
  ...
  const anchorScreen = (li: number): { x: number; y: number; visible: boolean } => {
    if (li < 0 || li >= LANGUAGES.length) return { x: 0, y: 0, visible: false };
    const lang = LANGUAGES[li];
    if (!lang) return { x: 0, y: 0, visible: false };
    const [ax, ay, az] = latLonToVec3(lang.anchor[1], lang.anchor[0], 1);
    if (!meshRef.current) return { x: 0, y: 0, visible: false };
  ...
  useEffect(() => {
    ...
    const onUp = (e: PointerEvent) => {
      ...
      if (st.down && !wasDrag && performance.now() - st.down.t < 600) {
        ...
        const selectedLang = li >= 0 ? LANGUAGES[li] : undefined;
        propsRef.current.onSelect(selectedLang ? selectedLang.id : null);
      }
      st.down = null;
    };
    const onLeave = () => {
  ...
  useFrame((_, dtRaw) => {
    ...
    if (st.pendingHover && !st.dragging) {
      ...
      const hoveredLang = li >= 0 ? LANGUAGES[li] : undefined;
      p.onHover?.(hoveredLang ? hoveredLang.id : null, x, y);
    ...
    if (req && req.stamp !== st.flyStamp) {
      ...
      if (li != null && li >= 0 && li < LANGUAGES.length) {
        const lang = LANGUAGES[li];
        if (lang) {
          const v = latLonToVec3(lang.anchor[1], lang.anchor[0], 1);
          const targetY = -Math.atan2(v[0], v[2]);
          const targetX = clamp((lang.anchor[1] * Math.PI) / 180, -PITCH_MAX, PITCH_MAX);
    ...
    /* Konstante Autorotation (immer aktiv, wenn nicht interagiert wird) */
    st.idle += dt;
    if (p.autorotate !== false && !st.reduced && !st.dragging && !st.fly && st.hoverIdx < 0) {
      yawRef.current.rotation.y += ((p.speed ?? 1.5) * Math.PI / 180) * dt;
    }
    ...
    for (let i = 0; i < st.heats.length; i++) {
      ...
      const heatVal = st.heats[i];
      if (heatVal !== undefined) {
        const d = target - heatVal;
        if (Math.abs(d) > 0.003) { st.heats[i] = heatVal + d * f; heatMoving = true; }
      }
    }
    ...
    if (heatMoving || key !== st.lastPaintKey) {
      ...
      if (mIdx >= 0 && mIdx < LANGUAGES.length) {
        const lang = LANGUAGES[mIdx];
        if (lang) {
          markers.push({
            lonlat: lang.anchor,
            color: p.dark ? lang.bright : lang.color,
          });
        }
      }
...
function GlobeEffects({ dark }: { dark: boolean }) {
  const limbColor = dark ? '#020309' : '#8c694e';
  const limbAlpha = dark ? 0.25 : 0.18;

  const glareColor = dark ? '#93c7d9' : '#ffffff';
  const glareAlpha = dark ? 0.10 : 0.35;
  const glareBlend = dark ? THREE.CustomBlending : THREE.NormalBlending;
  const glareEq = THREE.AddEquation;
  const glareSrc = dark ? THREE.OneFactor : THREE.SrcAlphaFactor;
  const glareDst = dark ? THREE.OneMinusSrcColorFactor : THREE.OneMinusSrcAlphaFactor;

  const limbMatRef = useRef<THREE.ShaderMaterial>(null!);
  const { camera, size, gl } = useThree();

  /* Pro-Frame: Kugelzentrum + Radius in Framebuffer-Pixel projizieren */
  const tmpCenter = useMemo(() => new THREE.Vector3(), []);
  const tmpEdge = useMemo(() => new THREE.Vector3(), []);

  useFrame(() => {
    const mat = limbMatRef.current;
    if (!mat) return;
    ...
    tmpEdge.set(1, 0, 0).project(camera);
    const ex = (tmpEdge.x * 0.5 + 0.5) * w;
  ...
  return (
    <>
      {/* Kugel-Limb-Schatten (Screen-Space-Vignette, äußere 22 %) */}
      <mesh scale={1.002}>
        ...
        <shaderMaterial
          ...
          uniforms={{
            c: { value: new THREE.Color(limbColor) },
            a: { value: limbAlpha },
          ...
          fragmentShader={`
            ...
            void main() {
              ...
              float f = smoothstep(0.92, 1.0, d);
              gl_FragColor = vec4(mix(vec3(1.0), c, f * a), 1.0);
            }
          `}
        />
      </mesh>
      ...
      <mesh scale={1.006}>
        ...
        <shaderMaterial
          transparent depthWrite={false}
          blending={glareBlend} blendEquation={glareEq} blendSrc={glareSrc} blendDst={glareDst}
          uniforms={{ c: { value: new THREE.Color(glareColor) }, a: { value: glareAlpha } }}
          vertexShader={`
...
export const LanguageGlobe = forwardRef<GlobeHandle, LanguageGlobeProps>(
  function LanguageGlobe(props, ref) {
    ...
    return (
      ...
      >
        {world && <Scene world={world} propsRef={propsRef} flyReqRef={flyReqRef} dark={props.dark}></Scene>}
      </Canvas>
    );
  },
);
