  ...
  return (
    <div className="w-full mt-10 sm:mt-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {/* 1) Featured Hero Bento Card: Produktfinder */}
        {featuredTool && (
          <div className="relative group rounded-3xl border border-primary/40 bg-card p-6 sm:p-7 shadow-sm hover:border-primary transition-all duration-300 flex flex-col justify-between overflow-hidden">
            <div>
              {/* Header Badge */}
              <div className="flex items-center justify-between gap-3 mb-4">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-primary/30 bg-primary/10 text-primary text-xs font-mono font-bold uppercase tracking-wider">
              ...
              {/* Title & Description */}
              <h3 className="font-heading font-extrabold text-lg sm:text-xl text-foreground mb-2 tracking-tight group-hover:text-primary transition-colors">
                {featuredTool.t}
              </h3>
              ...
              {featuredTool.chips && featuredTool.chips.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mb-6">
                  {featuredTool.chips.map((chip: string, idx: number) => (
                    <span
                      key={idx}
                      className="px-2.5 py-0.5 rounded-lg bg-background-subtle border border-card-border text-[11px] font-mono font-medium text-foreground/80"
                    >
                      {chip}
                    </span>
                  ))}
            ...
            <div className="pt-2 border-t border-card-border/60">
              <Link
                ...
                className="inline-flex items-center justify-between w-full py-2.5 px-4 rounded-xl bg-primary text-primary-foreground font-heading font-bold text-sm shadow-sm hover:bg-primary-hover active:scale-[0.98] transition-all duration-200"
              >
