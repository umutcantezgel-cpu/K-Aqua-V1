    ...
    <main className="k-page" style={{ paddingTop: 72 }}>
      ...
      <section className="k-section" style={{ paddingTop: 0 }}>
        <div className="k-container">
          ...
          <PipeFX variant="droplet" size={260} />
...
/* =====================  QUICK CONTACT FORM  ===================== */
function QuickContactForm({ L }) {
  const { useState: uS, useMemo: uM } = React;
  const [f, setF] = useDraft('quick-contact', { name: '', email: '', topic: (L.topics || [])[0] || '', msg: '' });
  const [sent, setSent] = uS(false);
  const set = (k, v) => setF((o) => ({ ...o, [k]: v }));
  const valid = f.name.trim() && /.+@.+\..+/.test(f.email) && f.msg.trim();

  const send = () => {
    const body = [`${L.fName}: ${f.name}`, `${L.fEmail}: ${f.email}`, `${L.fTopic}: ${f.topic}`, '', f.msg].join('\n');
    window.open('mailto:info@k-aqua.de?subject=' + encodeURIComponent(L.mailSubject + ' \u2014 ' + f.topic) + '&body=' + encodeURIComponent(body), '_blank');
    setSent(true);
  };

  if (sent) {
    return (
      <BentoCard tint style={{ height: '100%', alignItems: 'center', textAlign: 'center', gap: 'var(--sp-3)', justifyContent: 'center' }}>
        <div className="k-icon-chip" style={{ width: 56, height: 56 }}><Icons.Check size={26} /></div>
        <h3 className="k-h3">{L.doneTitle}</h3>
        <p className="k-body">{L.doneText}</p>
        <button type="button" className="k-link" onClick={() => setSent(false)}>{L.send}</button>
      </BentoCard>
    );
  }

  return (
    <BentoCard style={{ height: '100%', gap: 'var(--sp-4)' }}>
      <div className="k-icon-chip"><Icons.Mail size={24} /></div>
      <div>
        <h3 className="k-h3">{L.formTitle}</h3>
        <p className="k-body" style={{ marginTop: 4 }}>{L.formLead}</p>
      </div>
      ...
      <label className="k-field">
        <span>{L.fTopic}</span>
        <select className="k-input" value={f.topic} onChange={(e) => set('topic', e.target.value)}>
          {(L.topics || []).map((t) => <option key={t} value={t}>{t}</option>)}
        </select>
      </label>
      <label className="k-field"><span>{L.fMsg} *</span><textarea className="k-input" rows="4" value={f.msg} onChange={(e) => set('msg', e.target.value)}></textarea></label>
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-3)', flexWrap: 'wrap', marginTop: 'auto' }}>
        <KButton onClick={send} disabled={!valid} icon={<Icons.ArrowUpRight size={18} />}>{L.send}</KButton>
        <span className="k-body" style={{ fontSize: 12.5 }}>{L.privacy}</span>
      </div>
    </BentoCard>
  );
}
...
function ContactView() {
  ...
  return (
    <main className="k-page" style={{ paddingTop: 72 }}>
      <PageHero eyebrow={L.eyebrow} title={<span>{L.title1} <span className="k-grad-text">{L.titleGrad}</span></span>} />
      <section className="k-section" style={{ paddingTop: 0 }}>
        <div className="k-container" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1.15fr)', gap: 'var(--sp-6)', alignItems: 'start' }}>
